...
public class ItemEmptyTankard extends BaseItem
{
	...
	public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side, float px, float py, float pz)
	{
		 ...
		 * (world.getBlock(x, y, z) == InitBlocks.blockBarrel &&
		 * tile.getBeerType() != "empty" && tile.getBeerLevel() > 0) {
		 * tile.setBeerLevel(tile.getBeerLevel() - 1); if (tile.getBeerLevel()
		 * == 0) { tile.setBeerType("empty"); }
		 * player.destroyCurrentEquippedItem();
		 * player.inventory.addItemStackToInventory(new
		 * ItemStack(InitItems.itemFullTankard)); return true; }
		 */
